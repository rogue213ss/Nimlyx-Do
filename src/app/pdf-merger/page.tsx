...
export const metadata: Metadata = tool ? buildToolMetadata(tool, "/pdf-merger/") : {};

export default function PdfMergerPage() {
  if (!tool) return notFound();
  const category = getCategoryBySlug(tool.category);
  const path = "/pdf-merger/";
    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
      ...
      <section className="mt-14 max-w-prose" aria-labelledby="how-it-works">
        ...
        <p className="mt-2 text-ink/70">
          Select or drop in your PDF files, use the up/down controls to put
          them in the order you want, then merge. The result downloads as a
          single <code>merged.pdf</code> — pages are copied directly from
          your originals, not rendered to images, so quality and
          selectable text are preserved exactly.
        </p>
      </section>
      ...
      <section className="mt-12 max-w-prose" aria-labelledby="privacy">
        ...
        <p className="mt-2 text-ink/70">
          Your PDFs are processed entirely on your device. They never leave

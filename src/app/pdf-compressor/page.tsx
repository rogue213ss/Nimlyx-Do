...
export const metadata: Metadata = tool ? buildToolMetadata(tool, "/pdf-compressor/") : {};

export default function PdfCompressorPage() {
  if (!tool) return notFound();
  const category = getCategoryBySlug(tool.category);
  const path = "/pdf-compressor/";
    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
      ...
      <section className="mt-14 max-w-prose" aria-labelledby="how-it-works">
        ...
        <p className="mt-2 text-ink/70">
          Choose a compression level and upload your PDF. Embedded JPEG
          images are recompressed at a lower quality, and the file is
          rebuilt to remove unnecessary overhead — all directly in your
          browser. If a PDF is already well optimized, you&apos;ll see that
          plainly rather than a misleading &ldquo;compressed&rdquo; file
          that&apos;s actually the same size or larger.
        </p>
      </section>
      ...
      <section className="mt-12 max-w-prose" aria-labelledby="privacy">
        ...
        <p className="mt-2 text-ink/70">
          Your PDF is processed entirely on your device. It never leaves
